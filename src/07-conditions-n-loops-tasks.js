...
function isCreditCardNumber(ccn) {
  ...
  const checkNumber = Number(ccnArr.pop());
  let str = '';
  ccnArr.reverse();
  for (let i = 0; i < ccnArr.length; i += 1) {
    if (i % 2 === 0) {
      str += String(+ccnArr[i] * 2);
    } else {
      str += ccnArr[i];
    }
  }
  const sum = str.split('').map((x) => Number(x)).reduce((acc, item) => acc + item, 0);
  const mod = sum % 10;
  return checkNumber === (mod && 10) - mod;
}
...
function getDigitalRoot(num) {
  ...
  return getDigitalRoot(str.split('').reduce((acc, item) => +item + acc, 0));
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathes.length; i += 1) {
    newPathes.push(pathes[i].split('/'));
  }
  let str = '';
  for (let i = 0; i <= newPathes[0].length; i += 1) {
    const part = newPathes[0][i];
    if (!newPathes.every((item) => item[i] === part)) {
      break;
    }
    str += `${part}/`;
  }
